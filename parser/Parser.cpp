...
void Parser::nextToken(unsigned line, const std::string& token, Schema& schema) {
    ...
    case State::Index:
        if (isIdentifier(tok)) {
            ...
            // temporarely store the index in the latest parsed relation, since we don't know the table name yet
            // we'll move it to the correct table in State::IndexOn
            schema.relations.back().indices.push_back(Schema::Relation::Index(token));
        } else {
    ...
    case State::IndexName:
        if (tok == keyword::On) {
            ...
            throw ParserError(line, "Expected index 'ON', found '" + token + "'");
        }
        break;
    case State::IndexOn:
        if (isIdentifier(tok)) {
            // move the index to the right table
            auto index = schema.relations.back().indices.back();
            schema.relations.back().indices.pop_back();
            // find the corresponding table by name
            auto table = std::find_if(schema.relations.begin(), schema.relations.end(), [&token](Schema::Relation & relation) {
                return relation.name == token;
            });
            if (table == schema.relations.end()) {
                throw ParserError(line, "No such table '" + token + "'");
            }
            // rotate the table to the end, so we'll find it again
            std::rotate(table, table + 1, schema.relations.end());

            // now operate on the back() element again
            schema.relations.back().indices.push_back(index);
    ...
    case State::IndexListBegin:
        if (isIdentifier(tok)) {
            ...
            schema.relations.back().indices.back().keys.push_back(std::distance(attributes.begin(), it));
    ...
    case State::AttributeName:
        ...
        } else if (tok == keyword::Char || tok == keyword::Varchar) {
